...
exports.exploreLatest = async(req, res) => {
  ...
  try {
    ...
    res.render('explore-latest', { title: 'Cooking blog - Explore Lastest', recipe});
  } catch (error) {
    res.satus(500).send({mesage: error.message || "Error Occured"})
  }
}
 ...
 */
 exports.register = async(req, res) => {
  const infoErrorsObj = req.flash('infoErrors');
  const infoSubmitObj = req.flash('infoSubmit');
  res.render('register', { title: 'Cooking blog - Register page', infoErrorsObj, infoSubmitObj });
}
/** 
 * POST/register
 * register On Post
...
async function updateRecipe(){
  try {
    const res = await Recipe.updateOne({ name: 'Naan' }, { name: 'Indian Naan' });
    res.n;
    res.nModified
  } catch (error) {
    console.log(error);
